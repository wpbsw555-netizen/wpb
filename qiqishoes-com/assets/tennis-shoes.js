(() => {
  ...
  const TEXT = {
    zh: {
      slogan: '中国鞋具批发商', tabs: ['网球鞋链接', '时尚链接', '附件链接', '包包链接', '鞋子链接'], langs: ['中文', '英语', '西班牙语'],
      intro: '所有已迁移的分类、目录和产品图片都保存在本网站。点击产品进入本站详情页，再通过 WhatsApp 咨询价格。',
      categories: '产品分类目录', searchPh: '搜索产品名称或 ID', search: '搜索', loading: '正在加载本地产品目录…',
      loadingCategory: '正在加载该分类…', total: (n) => `共 ${n} 个产品`, categoryCount: (n) => `${n} 个产品`, pending: '迁移中',
      unavailable: '该分类正在迁移中，请稍后刷新。', empty: '没有找到匹配的产品', prev: '上一页', next: '下一页',
      page: (a, b) => `第 ${a} / ${b} 页`, wa: '在 WhatsApp 上聊天', allStats: (a, b) => `已迁移 ${a} 个分类，共 ${b} 个产品`,
    },
    en: {
      slogan: 'Chinese Footwear Wholesaler', tabs: ['Sneakers', 'Fashion', 'Accessories', 'Bags', 'Shoes'], langs: ['Chinese', 'English', 'Spanish'],
      ...
      categories: 'Product Category Directory', searchPh: 'Search product name or ID', search: 'Search', loading: 'Loading local product catalog…',
      loadingCategory: 'Loading this category…', total: (n) => `${n} products`, categoryCount: (n) => `${n} products`, pending: 'Importing',
      unavailable: 'This category is still being imported. Please refresh later.', empty: 'No matching products found', prev: 'Previous', next: 'Next',
      page: (a, b) => `Page ${a} / ${b}`, wa: 'Chat on WhatsApp', allStats: (a, b) => `${a} categories imported · ${b} products`,
    },
    es: {
      slogan: 'Mayorista chino de calzado', tabs: ['Zapatillas', 'Moda', 'Accesorios', 'Bolsos', 'Zapatos'], langs: ['Chino', 'Inglés', 'Español'],
      ...
      categories: 'Directorio de categorías', searchPh: 'Buscar nombre o ID', search: 'Buscar', loading: 'Cargando catálogo local…',
      loadingCategory: 'Cargando esta categoría…', total: (n) => `${n} productos`, categoryCount: (n) => `${n} productos`, pending: 'Importando',
      unavailable: 'Esta categoría todavía se está importando. Actualice la página más tarde.', empty: 'No se encontraron productos', prev: 'Anterior', next: 'Siguiente',
      page: (a, b) => `Página ${a} / ${b}`, wa: 'Chat por WhatsApp', allStats: (a, b) => `${a} categorías importadas · ${b} productos`,
    },
  };

  let lang = 'zh';
  let manifest = { categories: [], defaultCategory: '3551883', importedCategoryCount: 0, totalProducts: 0 };
  let categoryData = { category: {}, products: [] };
  let selectedCategory = new URLSearchParams(location.search).get('category') || '3551883';
  let query = '';
  let page = 1;
  const cache = new Map();
  ...
  function categoryName(meta) {
    return meta?.name || `Category ${selectedCategory}`;
  }
  ...
  function setLanguage(value) {
    ...
    document.querySelector('#categoryTitle').textContent = `${t.categories} · ${t.allStats(manifest.importedCategoryCount || 0, manifest.totalProducts || 0)}`;
  ...
  function renderCategories() {
    ...
    list.innerHTML = manifest.categories.map(category => {
      ...
      const pending = category.imported ? '' : ' pending';
      const cover = category.cover ? `<span class="category-thumb"><img src="${assetUrl(category.cover)}" alt="" loading="lazy" onerror="this.style.display='none'"></span>` : '<span class="category-thumb category-thumb-empty">•••</span>';
      const count = category.imported ? t.categoryCount(Number(category.count || 0)) : t.pending;
      return `<button type="button" class="category-chip${active}${pending}" data-category="${escapeHtml(category.id)}">${cover}<span class="category-copy"><strong>${escapeHtml(category.name)}</strong><small>${escapeHtml(count)}</small></span></button>`;
    }).join('');
  ...
  async function init() {
    ...
    try {
      ...
      if (Array.isArray(data.products)) {
        const legacy = {
          id: '3551883', name: data.title?.zh || 'AIR JORDAN3 乔丹3代', imported: true,
          count: data.products.length, cover: data.products[0]?.image || null, data: null,
        };
        manifest = { categories: [legacy, ...(data.categories || []).filter(item => String(item.id) !== '3551883')], defaultCategory: '3551883', importedCategoryCount: 1, totalProducts: data.products.length };
        categoryData = { category: legacy, products: data.products };
        cache.set('3551883', categoryData);
      } else {
        manifest = data;
      ...
      console.error(error);
      manifest = { categories: [], defaultCategory: '3551883', importedCategoryCount: 0, totalProducts: 0 };
    }
    if (!manifest.categories.some(item => String(item.id) === String(selectedCategory))) {
      selectedCategory = manifest.defaultCategory || manifest.categories.find(item => item.imported)?.id || '3551883';
    }
    setLanguage(lang);
    if (!cache.has(selectedCategory)) await loadCategory(selectedCategory);
  }

  document.querySelectorAll('[data-lang]').forEach(button => { button.onclick = () => setLanguage(button.dataset.lang); });
  document.querySelector('#catalogSearch').onsubmit = event => { event.preventDefault(); query = document.querySelector('#catalogQuery').value; page = 1; renderProducts(); };
  document.querySelector('#catalogQuery').oninput = event => { query = event.target.value; page = 1; renderProducts(); };
