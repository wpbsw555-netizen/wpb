(() => {
  ...
  const IMAGE_DATA = new URL('./aj4-black.webp.b64?v=202608072320', script.src).href;
  const COPY = {
  ...
  const lang=()=>{const v=(document.documentElement.lang||'zh').toLowerCase();return v.startsWith('en')?'en':v.startsWith('es')?'es':'zh'};

  const css=document.createElement('style');
  css.id='aj4-home-layout-v4';
  css.textContent=`
    ...
    .hero-shoe{display:block!important;width:100%!important;height:100%!important;object-fit:contain!important;object-position:center!important;opacity:1!important;visibility:visible!important}
  ...
  product.innerHTML=`
    <div class="hero-shoe-wrap"><img class="hero-shoe" alt=""></div>
  ...
  fetch(IMAGE_DATA,{cache:'no-store'})
    .then(r=>{if(!r.ok)throw new Error(`image data ${r.status}`);return r.text()})
    .then(text=>{
      const b64=text.replace(/\s+/g,'');
      if(!b64.startsWith('UklGR')) throw new Error('invalid WebP base64 header');
      const src='data:image/webp;base64,'+b64;
      image.onload=()=>{
        const card=document.querySelector('.dept.sneakers');
        if(card) card.style.setProperty('--image',`url("${src}")`);
      };
      image.onerror=()=>console.error('AJ4 image decode failed');
      image.src=src;
    })
    .catch(err=>console.error('AJ4 image load failed',err));
})();
