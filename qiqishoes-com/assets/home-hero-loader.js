(() => {
  ...
  const IMAGE_PARTS = [0,1,2,3,4].map(i => new URL(`./aj4-hero.part${i}`, script.src).href);
  ...
  function styleHero(){
    ...
    s.textContent=`
      ...
      .hero-shoe{display:block;width:100%;height:100%;object-fit:contain;object-position:center;opacity:1!important;filter:none!important;transform:none!important}
      ...
      @media(max-width:980px){.hero{grid-template-columns:45% 55%!important;min-height:680px!important}.hero-content{min-height:680px!important;padding:44px 24px 40px 30px!important}.hero-product{padding:20px 30px 28px 4px}.hero-shoe-wrap{height:350px}.hero-sales{margin-left:24px}}
      @media(max-width:720px){.hero{display:block!important;min-height:auto!important}.hero-content{min-height:auto!important;padding:36px 22px 30px!important}.hero-product{width:100%;padding:12px 18px 30px}.hero-shoe-wrap{height:300px}.hero-sales{margin:6px 4px 0}.hero::after{right:9px;top:18px;bottom:auto;font-size:10px;letter-spacing:3px}}
    `;
  ...
  product.innerHTML=`
    <div class="hero-shoe-wrap"><img class="hero-shoe" decoding="sync" fetchpriority="high" alt=""></div>
  ...
  Promise.all(IMAGE_PARTS.map(async url=>{
    ...
    if(card) card.style.setProperty('--image',`url("data:image/webp;base64,${base64}")`);
  }).catch(err=>console.error('AJ4 IMAGE LOAD ERROR',err));
})();
