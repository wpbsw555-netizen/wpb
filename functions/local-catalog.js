...
function renderPage({ dept, id, title, allItems, items, origin, page, pages, fallbackId }) {
  ...
  const cards = items.map((item, offset) => {
    ...
    const priority = offset < 6 ? 'eager' : 'lazy';
    const fetchPriority = offset < 4 ? 'high' : 'auto';
    return `<article class="product-card">
      ...
      <div class="product-meta">
        ...
        <span>ID ${esc(id)}-${globalIndex + 1}</span>
...
<main class="wrap"><div class="eyebrow">${esc(title)}</div><div class="head"><h1>产品图片 / Product Images</h1><p>分类 ID ${esc(id)} · 共 ${allItems.length} 张 · 每页最多 ${PAGE_SIZE} 张</p></div>${pager}${items.length ? `<section class="grid">${cards}</section>` : '<div class="empty">此分类暂时没有读取到图片，请稍后刷新。</div>'}${pager}</main>
...
<script>
(()=>{
  document.querySelectorAll('.product-card img').forEach(im=>{
    im.addEventListener('error',()=>{
      if(im.dataset.stage!=='local'){
        im.dataset.stage='local';
        im.src=im.dataset.fallback;
        return;
      }
      if(im.dataset.stage!=='default'){
        im.dataset.stage='default';
        im.src=im.dataset.default;
        return;
      }
      im.style.opacity='.35';
    });
  });
  const modal=document.getElementById('modal'),img=document.getElementById('modalImg'),name=document.getElementById('modalName');
  document.querySelectorAll('.image-button').forEach(b=>b.addEventListener('click',()=>{const shown=b.querySelector('img');img.src=shown?.src||b.dataset.full;img.alt=b.dataset.name||'';name.textContent=b.dataset.name||'';modal.classList.add('open');modal.setAttribute('aria-hidden','false')}));
  function close(){modal.classList.remove('open');modal.setAttribute('aria-hidden','true');img.src=''}
  document.querySelector('.close').onclick=close;modal.addEventListener('click',e=>{if(e.target===modal)close()});addEventListener('keydown',e=>{if(e.key==='Escape')close()});
})();
</script>
</body></html>`;
}
...
export async function onRequest(context) {
  ...
  normalized.searchParams.set('dept', dept);
  normalized.searchParams.set('id', id);
  normalized.searchParams.set('page', String(requestedPage));
  ...
  try {
    upstream = await fetch(target.href, {
      redirect: 'follow',
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; QIQI-Catalog/2.0)',
  ...
  const pages = Math.max(1, Math.ceil(collector.items.length / PAGE_SIZE));
  const page = Math.min(requestedPage, pages);
  const start = (page - 1) * PAGE_SIZE;
  const visibleItems = collector.items.slice(start, start + PAGE_SIZE);
  const origin = incoming.origin;
  const html = renderPage({ dept, id, title: config.label, allItems: collector.items, items: visibleItems, origin, page, pages, fallbackId: config.fallback });
